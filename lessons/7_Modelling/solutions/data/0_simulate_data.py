...
# 1. Linear dataset (Force vs Position): F = kx + noise
...
# Save as CSV
linear_df.to_csv("1_force_wire.csv", index = False)



# 2. Quadratic dataset (Accelerated motion): x(t) = (1/2) a t^2 + noise
...
quadratic_df = pd.DataFrame({
    "time (s)": t,
    "position (m)": position_quad
})

# Save as TSV
quadratic_df.to_csv("2_accelerated_motion.tsv", sep = "\t", index = False)



# 3. Sinusoidal dataset (Harmonic motion): x(t) = A sin(ωt) + noise
...
sinusoidal_df = pd.DataFrame({
    "time (s)": t2,
    "position (m)": position_sin
})

# Save as JSON
sinusoidal_df.to_json("3_harmonic_motion.json", orient = "records", indent = 2)

print("Datasets generated:")
print("1_force_wire.csv")
print("2_accelerated_motion.tsv")
print("3_harmonic_motion.json")
